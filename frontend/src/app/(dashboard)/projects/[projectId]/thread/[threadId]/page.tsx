...
export default function ThreadPage({
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | NexI`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive agent conversation powered by NexI`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | NexI`);
      }
  ...
  useEffect(() => {
    if (initialLoadCompleted && subscriptionData) {
      const hasSeenUpgradeDialog = localStorage.getItem('NexI_upgrade_dialog_displayed');
  ...
  const handleDismissUpgradeDialog = () => {
    ...
    localStorage.setItem('NexI_upgrade_dialog_displayed', 'true');
  };
        ...
        <div
          ...
          <div className={cn(
            ...
            <ChatInput
              ...
              placeholder={`Ask ${agent ? agent.name : 'NexI'} anything...`}
              loading={isSending}
