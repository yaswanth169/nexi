...
def check_NexI_directory():
    """Check if we're in a NexI repository"""
    required_dirs = ['backend', 'frontend']
    required_files = ['README.md', 'docker-compose.yaml']
    
    for directory in required_dirs:
        if not os.path.isdir(directory):
            print_error(f"'{directory}' directory not found. Make sure you're in the NexI repository root.")
            return False
    
    for file in required_files:
        if not os.path.isfile(file):
            print_error(f"'{file}' not found. Make sure you're in the NexI repository root.")
            return False
    
    print_success("NexI repository detected")
    return True
...
def collect_daytona_info():
    ...
    print_info(f"Enter 'kortix/NexI:0.1.3' as the image name")
...
def collect_llm_api_keys():
    ...
    print_info("You need at least one LLM provider API key to use NexI")
...
def configure_backend_env(env_vars, use_docker=True):
    ...
    env_content = """# Generated by NexI setup script
...
def start_NexI():
    """Start NexI using Docker Compose or manual startup"""
    print_info("You can start NexI using either Docker Compose or by manually starting the frontend, backend and worker.")

    print(f"\n{Colors.CYAN}How would you like to start NexI?{Colors.ENDC}")
    ...
    if use_docker:
        print_info("Starting NexI with Docker Compose...")
...
def final_instructions(use_docker=True, env_vars=None):
    ...
    print(f"\n{Colors.GREEN}{Colors.BOLD}✨ NexI Setup Complete! ✨{Colors.ENDC}\n")
    ...
    if env_vars and 'llm' in env_vars and 'MODEL_TO_USE' in env_vars['llm']:
        ...
        print_info(f"NexI is configured to use {Colors.GREEN}{default_model}{Colors.ENDC} as the default LLM model")
    
    if use_docker:
        print_info("Your NexI instance is now running!")
        print_info("Access it at: http://localhost:3000")
        print_info("Create an account using Supabase authentication to start using NexI")
        print("\nUseful Docker commands:")
        print(f"{Colors.CYAN}  docker compose ps{Colors.ENDC}         - Check the status of NexI services")
        ...
        print(f"{Colors.CYAN}  docker compose down{Colors.ENDC}       - Stop NexI services")
        print(f"{Colors.CYAN}  docker compose up -d{Colors.ENDC}      - Start NexI services (after they've been stopped)")
    else:
        print_info("NexI setup is complete but services are not running yet.")
        print_info("To start NexI, you need to:")
        ...
        print_info("4. Once all services are running, access NexI at: http://localhost:3000")
        print_info("5. Create an account using Supabase authentication to start using NexI")
...
def main():
    ...
    print_banner()
    print("This wizard will guide you through setting up NexI, an open-source generalist AI agent.\n")
    ...
    if current_step <= 1:
        ...
        if not check_NexI_directory():
            print_error("This setup script must be run from the NexI repository root directory.")
            sys.exit(1)
    ...
    if current_step <= 8:
        ...
        print_step(current_step, total_steps, "Starting NexI")
        use_docker = start_NexI()
        if not use_docker:
